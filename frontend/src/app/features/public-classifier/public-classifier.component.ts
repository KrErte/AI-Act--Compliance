...
@Component({
  selector: 'app-public-classifier',
  standalone: true,
  ...
  template: `
    <div class="container">
      @if (loadingQuestions) {
        ...
        <div class="result">
          <h1>{{ 'classification.result_title' | translate }}</h1>
          ...
          <div class="cta-section card">
            <p>{{ 'classification.sign_up_cta' | translate }}</p>
            <a routerLink="/auth/register" class="btn btn-primary">{{ 'common.sign_up' | translate }}</a>
          </div>
          <button class="btn btn-secondary" (click)="startOver()">{{ 'classification.start_over' | translate }}</button>
        </div>
      } @else {
        <div class="hero">
          <h1>{{ 'classification.public_title' | translate }}</h1>
          <p>{{ 'classification.public_subtitle' | translate }}</p>
        </div>
  ...
  styles: [`
    ...
    .progress-bar { height: 4px; background: var(--bg-tertiary); border-radius: 2px; margin-bottom: 0.5rem; max-width: 700px; margin-left: auto; margin-right: auto; }
    .progress-fill { height: 100%; background: var(--primary); border-radius: 2px; transition: width 0.3s; }
    .progress-text { font-size: 0.875rem; color: var(--text-secondary); margin-bottom: 1.5rem; text-align: center; }
    ...
    .nav-buttons { display: flex; justify-content: space-between; max-width: 700px; margin: 1.5rem auto 0; }
    .result { max-width: 700px; margin: 0 auto; }
    .result h1 { font-size: 1.5rem; margin-bottom: 1rem; }
    ...
    .cta-section { text-align: center; padding: 1.5rem; margin-bottom: 1rem; }
    .cta-section p { margin-bottom: 1rem; }
    .label { font-size: 0.875rem; color: var(--text-secondary); }
  `]
})
export class PublicClassifierComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<any>(`${this.apiUrl}/public/classify/questions`).subscribe({
      next: res => {
  ...
  next(): void { if (this.currentIndex < this.visibleQuestions.length - 1) this.currentIndex++; }
  prev(): void { if (this.currentIndex > 0) this.currentIndex--; }

  submit(): void {
    this.submitting = true;
    this.http.post<any>(`${this.apiUrl}/public/classify`, { answers: this.answers }).subscribe({
